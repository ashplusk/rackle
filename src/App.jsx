...
// DEAL DIFFICULTY
function dealDiff(hand){
  const scores=SECS.map(s=>s.ck(hand)),best=Math.max(...scores),viable=scores.filter(s=>s>0.08).length;
  const c={};hand.forEach(t=>{const k=JSON.stringify(t);c[k]=(c[k]||0)+1;});
  const pr=Object.values(c).filter(v=>v>=2).length,pg=Object.values(c).filter(v=>v>=3).length;
  const jk=hand.filter(t=>t.t==="j").length,fl=hand.filter(t=>t.t==="f").length;
  const ease=best*2+viable*0.08+pr*0.06+pg*0.1+jk*0.12+fl*0.05;
  if(ease>=0.7)return{l:"Easy Deal",e:"🌿",c:"#1B7D4E"};
  if(ease>=0.45)return{l:"Fair Deal",e:"🃏",c:"#B08A35"};
  if(ease>=0.25)return{l:"Tough Deal",e:"🔥",c:"#D48A2A"};
  return{l:"Brutal Deal",e:"💀",c:"#B83232"};
}
...
function Settings({home,settings,setSettings,showTutorial}){
  const clearHistory=()=>{
    if(window.confirm("Clear all your game history and stats? This cannot be undone.")){
  ...
  return(
    <div style={S.pg} className="rk-pg">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <button onClick={home} style={S.back} aria-label="Back to home">← Back</button>
        <span style={{fontFamily:F.d,fontSize:18,fontWeight:800,color:C.ink}}>⚙️ Settings</span>
        <div style={{width:50}}/>
      </div>
      <div style={S.card}>
      ...
      <div style={S.card}>
        ...
        <Row label="Clear History" sub="Reset all stats and streaks">
          <button onClick={clearHistory} style={{padding:"6px 12px",borderRadius:8,border:`1px solid ${C.cinn}30`,background:C.cinn+"08",fontSize:11,cursor:"pointer",fontWeight:600,color:C.cinn}}>Clear</button>
        </Row>
      </div>
      <div style={S.card}>
...
function Tutorial({onDone,onBack}){
  ...
  return(
    <div style={S.pg} className="rk-pg">
      {onBack&&<button onClick={onBack} style={S.back} aria-label="Back to home">← Back</button>}
    ...
    <AppShell settings={settings}>
      ...
      {showFirstPayoff&&firstPayoffRes
        ? <FirstPayoff
          ...
          />
        : <>
            {screen==="home"&&<Home {...{streak,rounds,dDone,dRes,showHelp,setShowHelp,go,settings}} showStats={()=>setScreen("stats")} showSettings={()=>setScreen("settings")} showTutorial={()=>setScreen("tutorial")}/>}
...
// HOME
function Home({streak,rounds,dDone,dRes,showHelp,setShowHelp,go,showStats,showSettings,showTutorial,settings}){
  const [copied,setCopied]=useState(false);
  const dn=getDayNum(),wk=getWeekly(),yd=getYesterday(),stats=getStats();
  const streakBadge=getStreakBadge(streak);
  const stxt=dRes?`🀄 Rackle #${dn}\n${dRes.rating} ${dRes.emoji}${dRes.dd?` · ${dRes.dd}`:""}\n${dRes.section}${dRes.time?` · ${fT(dRes.time)}`:""}\nplayrackle.com`:"";
  const cp=()=>{navigator.clipboard?.writeText(stxt).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2000);});};
  return(
    <div style={S.pg} className="rk-pg">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
        <div style={{fontFamily:F.d,fontSize:14,fontWeight:800,color:C.ink,letterSpacing:-0.5}}>Rackle</div>
        <button onClick={showSettings} aria-label="Open settings" style={{background:"none",border:`1px solid ${C.bdr}`,borderRadius:8,padding:"4px 12px",fontSize:12,fontWeight:600,color:C.mut,cursor:"pointer"}}>Settings</button>
      </div>
      <div style={{textAlign:"center",paddingTop:16,marginBottom:10}}>
        <div className="rk-float" style={{fontSize:44,marginBottom:8}}>🀄</div>
        ...
        <p style={{fontSize:13,color:C.mut,margin:"8px 0 0",lineHeight:1.6}}>Rate your Charleston. Track your improvement.</p>
      </div>

      {rounds===0&&<div style={{display:"flex",gap:6,justifyContent:"center",marginBottom:16,flexWrap:"wrap"}}>
        {["🀄 Daily deal","👥 Same hand for everyone","📈 Track your progress"].map((t,i)=>(
          <span key={i} style={{fontSize:11,color:C.mut,background:C.bg2,border:`1px solid ${C.bdr}`,borderRadius:20,padding:"3px 10px",fontWeight:600}}>{t}</span>
        ))}
      </div>}

      {(rounds>0||streak>0)&&<div style={{display:"flex",gap:6,marginBottom:12}}>
        {streak>0&&<Pill i={streakBadge?streakBadge.badge:"🔥"} v={streak} l={streakBadge?streakBadge.title.toUpperCase():"STREAK"} hl/>}
        <Pill i="🎲" v={rounds} l="ROUNDS"/>
      </div>}

      {yd&&dDone&&dRes&&(()=>{const tg=dRes.gi,yg=yd.gi,b=tg<yg,s=tg===yg;return(
        <div style={{...S.card,background:b?C.jade+"06":s?"#FFFFF8":C.cinn+"04",borderColor:b?C.jade+"20":s?C.gold+"20":C.cinn+"15",textAlign:"center",padding:12,marginBottom:8}}>
          <span style={{fontSize:16}}>{b?"⬆️":s?"➡️":"⬇️"}</span>
          <span style={{fontSize:12,fontWeight:600,color:b?C.jade:s?C.gold:C.cinn,marginLeft:8}}>{b?"Better than yesterday!":s?"Same as yesterday":"Yesterday was stronger"}</span>
        </div>);})()}
      ...
      {!dDone?(
        ...
        <div style={{...S.card,borderColor:C.jade+"35",background:"linear-gradient(145deg,#F0FAF5,#FAF7F1)",padding:20,borderRadius:16,marginBottom:8}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
            <div>
              <div style={{fontFamily:F.d,fontSize:18,fontWeight:800,color:C.ink}}>Daily Rackle #{dn}</div>
              <div style={{fontSize:13,color:C.jade,fontWeight:600,marginTop:3}}>{dRes?.rating} {dRes?.emoji}</div>
              <div style={{fontSize:11,color:C.mut,marginTop:2}}>{dRes?.section}{dRes?.time?` · ⏱ ${fT(dRes.time)}`:""}</div>
              {dRes?.dd&&<div style={{fontSize:10,color:dRes.dc||C.mut,fontWeight:600,marginTop:3}}>{dRes.de} {dRes.dd}</div>}
            </div>
            <div style={{padding:"6px 12px",borderRadius:10,background:C.jade+"12",border:`2px solid ${C.jade}30`}}>
              <span style={{fontFamily:F.d,fontSize:11,fontWeight:800,color:C.jade}}>{dRes?.rating?.split(" ")[0]}</span>
            </div>
          </div>
          <div style={{padding:12,background:"#fff",borderRadius:10,border:`1px solid ${C.bdr}`,marginBottom:10}}>
            <div style={{fontFamily:"monospace",fontSize:12,color:C.ink,lineHeight:1.7,whiteSpace:"pre"}}>{stxt}</div>
          </div>
          <button onClick={cp} style={{...S.greenBtn,width:"100%",fontSize:13,padding:"12px 0"}}>{copied?"✓ Copied!":"📋 Copy & share with your mahj group"}</button>
        </div>
      )}

      <button onClick={()=>go("free")} aria-label="Play Practice Mode" style={{width:"100%",padding:"20px",borderRadius:18,border:"none",cursor:"pointer",marginBottom:8,background:"linear-gradient(135deg,#B83232,#8A2020)",color:"#fff",display:"flex",alignItems:"center",gap:16,textAlign:"left",boxShadow:"0 8px 28px rgba(184,50,50,0.28)"}}>
        <div aria-hidden="true" style={{width:52,height:52,borderRadius:15,background:"rgba(255,255,255,0.12)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>🀄</div>
        <div style={{flex:1}}>
          <div style={{fontSize:11,opacity:0.75,letterSpacing:2,fontWeight:700,marginBottom:3}}>UNLIMITED PLAY</div>
          <div style={{fontFamily:F.d,fontSize:22,fontWeight:800}}>Practice Mode</div>
          <div style={{fontSize:12,opacity:0.85,marginTop:3}}>Sharpen your Charleston strategy.</div>
        </div>
      </button>

      <div style={{display:"flex",gap:8,marginBottom:8}}>
        {wk&&<div style={{flex:1,borderRadius:14,padding:"12px",background:"#fff",border:`1px solid ${C.bdr}`}}>
          <div style={{fontSize:8,color:wk.color,letterSpacing:2,fontWeight:700,marginBottom:4}}>THIS WEEK'S FOCUS</div>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontSize:20}}>{wk.icon}</span>
            <div>
              <div style={{fontSize:13,fontWeight:800,color:C.ink,lineHeight:1.2}}>{wk.name}</div>
              <div style={{fontSize:10,color:C.mut,marginTop:2,lineHeight:1.4}}>{wk.desc}</div>
            </div>
          </div>
          <div style={{marginTop:8,paddingTop:8,borderTop:`1px solid ${wk.color}15`,fontSize:10,color:wk.color,fontWeight:600}}>Practice this section in Free Play →</div>
        </div>}
        <button onClick={()=>setShowHelp(!showHelp)} aria-expanded={showHelp} aria-controls="help-panel"
          style={{flex:1,borderRadius:14,padding:"12px",background:showHelp?C.gold+"10":"#fff",border:`1px solid ${showHelp?C.gold+"40":C.bdr}`,cursor:"pointer",textAlign:"left"}}>
          <div style={{fontSize:8,color:C.gold,letterSpacing:2,fontWeight:700,marginBottom:4}}>HOW TO PLAY</div>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontSize:20}}>📖</span>
            <div>
              <div style={{fontSize:13,fontWeight:800,color:C.ink,lineHeight:1.2}}>Rules & Tips</div>
              <div style={{fontSize:10,color:C.mut,marginTop:2,lineHeight:1.4}}>Charleston rules & all 9 sections</div>
            </div>
          </div>
          <div style={{marginTop:8,paddingTop:8,borderTop:`1px solid ${C.gold}20`,fontSize:10,color:C.gold,fontWeight:600}}>{showHelp?"Tap to close ↑":"Tap to expand →"}</div>
        </button>
      </div>
      ...
      {rounds>=3&&<button onClick={showStats} aria-label="View my stats and section mastery" style={{width:"100%",cursor:"pointer",display:"flex",alignItems:"center",gap:14,marginBottom:8,borderRadius:16,padding:"14px 16px",textAlign:"left",background:"#fff",border:`1px solid ${C.bdr}`}}>
        <div style={{width:40,height:40,borderRadius:11,background:C.bg2,border:`1px solid ${C.bdr}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>📊</div>
        <div style={{flex:1}}>
          <div style={{fontSize:14,fontWeight:700,color:C.ink}}>My Stats</div>
          <div style={{fontSize:11,color:C.mut,marginTop:1}}>Section Mastery · Ratings · Streak History</div>
        </div>
        <span style={{fontSize:16,color:C.mut}}>→</span>
      </button>}

      <div style={{borderRadius:14,overflow:"hidden",marginBottom:8,border:`1px solid ${C.bdr}`}}>
        <div style={{background:C.jade+"08",padding:"11px 14px",borderBottom:`1px solid ${C.jade}15`,display:"flex",alignItems:"center",gap:10}}>
          <span style={{fontSize:18}}>👥</span>
          <div>
            <div style={{fontSize:12,fontWeight:700,color:C.ink}}>Same hand for every player, every day</div>
            <div style={{fontSize:11,color:C.mut,marginTop:1}}>Compare your Charleston with your whole club — no spoilers.</div>
          </div>
        </div>
        <div style={{background:"#fff",padding:"11px 14px",display:"flex",alignItems:"center",gap:10}}>
          <span style={{fontSize:18}}>📈</span>
          <div>
            <div style={{fontSize:12,fontWeight:700,color:C.ink}}>Daily reps build real instincts</div>
            <div style={{fontSize:11,color:C.mut,marginTop:1}}>5 minutes a day sharpens your passing strategy faster than anything else.</div>
          </div>
        </div>
      </div>
      <div style={{textAlign:"center",padding:"22px 0 8px",marginTop:16}}>
        ...
        <div style={{marginTop:12}}>
          <a href="mailto:hello@playrackle.com" style={{fontSize:11,color:C.mut,textDecoration:"none",borderBottom:`1px solid ${C.bdr}`}}>Contact</a>
        </div>
      </div>
    </div>
  );
  ...
  return(
    <div style={S.pg} className="rk-pg">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <button onClick={home} style={S.back} aria-label="Back to home">← Back</button>
        <span style={{fontFamily:F.d,fontSize:18,fontWeight:800,color:C.ink}}>My Rackle Stats</span>
        <div style={{width:50}}/>
      </div>
      {!stats?(
...
function Game({mode,home,onDone,settings}){
  ...
  useEffect(()=>{
    ...
    const dealt=d.slice(0,13);setHand(dealt);setPool(d.slice(13).filter(t=>t.t!=="j"));setDd(dealDiff(dealt));
  ...
  const confirm=()=>{
    ...
    onDone({rating:RATS[gi],emoji:REMO[gi],section:`${top.icon} ${top.name}`,sid:top.id,score:top.score,time:totalEl,gi,dd:dd?.l,de:dd?.e,dc:dd?.c});
    setPhase("result");
  };
  ...
  const restart=()=>{
    ...
    setDd(dealDiff(dealt));setPhase("deal");
    setTimeout(()=>{setPhase("pass");stRef.current=Date.now();setSt(Date.now());},500);
  };
  ...
  const resultData=(phase==="result"&&chosenSec)?()=>{
    ...
    const totalEl=Math.floor((elRef.current+(stRef.current?Date.now()-stRef.current:0))/1000);
    const stxt=`🀄 Rackle${isD?" #"+dn:""}\n${r} ${REMO[gi]}${dd?` · ${dd.e} ${dd.l}`:""}\n${top.icon} ${top.name} · ${pct}%\n⏱ ${fT(totalEl)}\nplayrackle.com`;
  ...
  return(
    <div style={S.pg} className="rk-pg">
      ...
      {phase==="result"&&rd&&(
        <>
          {/* HERO */}
          <div style={{textAlign:"center",paddingTop:8,paddingBottom:16,borderBottom:`1px solid ${C.bdr}`,marginBottom:14}}>
            <h2 style={{fontFamily:F.d,fontSize:22,color:C.ink,margin:"0 0 10px",fontWeight:800}}>{rd.isD?`Daily Rackle #${rd.dn}`:"Your Rackle Scorecard"}</h2>
            <div role="status" aria-live="polite" style={{display:"inline-block",padding:"8px 24px",borderRadius:14,background:rd.gc+"14",border:`2px solid ${rd.gc}35`,marginBottom:8}}>
              <span style={{fontFamily:F.d,fontSize:20,fontWeight:900,color:rd.gc}}>{REMO[rd.gi]} {rd.r}</span>
            </div>
            {dd&&<div style={{marginTop:8}}><span style={{fontSize:11,color:dd.c,fontWeight:600,background:dd.c+"10",padding:"2px 10px",borderRadius:8}}>{dd.e} {dd.l}</span></div>}
          </div>

          {/* STATS ROW */}
          <div style={{...S.card,padding:"12px 14px",display:"flex",justifyContent:"space-around",textAlign:"center",marginBottom:8}}>
            <div><div style={{fontSize:9,color:C.mut,letterSpacing:1.5,fontWeight:700,marginBottom:3}}>BEST SECTION</div><div style={{fontSize:13,color:C.ink,fontWeight:700}}>{rd.top.icon} {rd.top.name}</div></div>
            <div style={{width:1,background:C.bdr}}/>
            <div><div style={{fontSize:9,color:C.mut,letterSpacing:1.5,fontWeight:700,marginBottom:3}}>FIT SCORE</div><div style={{fontSize:13,color:rd.gc,fontWeight:800}}>{rd.pct}%</div></div>
            <div style={{width:1,background:C.bdr}}/>
            <div><div style={{fontSize:9,color:C.mut,letterSpacing:1.5,fontWeight:700,marginBottom:3}}>TIME</div><div style={{fontSize:13,color:C.ink,fontWeight:700}}>{fT(rd.totalEl)}</div></div>
          </div>

          {/* SHARE CARD — unified, moved up */}
          <div style={{...S.shareCard,padding:0,overflow:"hidden",marginBottom:8}}>
            <div style={{padding:"16px 20px"}}>
              <div style={{fontSize:8,color:C.jade,letterSpacing:4,fontWeight:700,marginBottom:6}}>SHARE YOUR RESULT</div>
              <div style={{fontFamily:F.d,fontSize:17,color:C.ink,fontWeight:800,marginBottom:4}}>{rd.isD?`Rackle #${rd.dn} · `:""}{rd.r} {REMO[rd.gi]}</div>
              {dd&&<div style={{fontSize:11,color:dd.c,fontWeight:600,marginBottom:4}}>{dd.e} {dd.l}</div>}
              <div style={{fontSize:11,color:C.mut,marginBottom:8}}>{rd.top.icon} {rd.top.name} · {rd.pct}% · ⏱ {fT(rd.totalEl)}</div>
              <div style={{background:C.bg2,borderRadius:8,padding:"8px 10px",fontFamily:"monospace",fontSize:11,color:C.ink,lineHeight:1.7,whiteSpace:"pre",marginBottom:10}}>{rd.stxt}</div>
              <button onClick={()=>{navigator.clipboard?.writeText(rd.stxt).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2000);});}} style={{...S.greenBtn,width:"100%",fontSize:12,padding:"10px 0"}} aria-label="Copy result to clipboard">{copied?"✓ Copied to clipboard!":"📋 Copy & share with your mahj group"}</button>
            </div>
          </div>
      ...
      {phase==="askCourtesy"&&<Ask icon="🤝" title="Courtesy Pass?" desc="Pass 1–3 tiles across." hand={hand} timer={getDisplayTime()} onSort={()=>setHand(sortHand(hand))} onNo={()=>{setSel([]);setNewIdx([]);setPhase("chooseHand");}} onYes={()=>{setSel([]);setNewIdx([]);setPhase("courtesy");}} large={large}/>}

      {phase==="courtesy"&&(
        <>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            <span aria-hidden="true" style={{fontSize:22}}>🤝</span>
            {getDisplayTime()&&<span aria-label={`Time elapsed: ${getDisplayTime()}`} style={{fontSize:12,color:C.mut,fontFamily:F.d,fontWeight:700}}>⏱ {getDisplayTime()}</span>}
          </div>
          ...
          <button
            onClick={()=>{if(sel.length<1)return;haptic(40);const pt=sel.map(i=>hand[i]);setPassed(p=>[...p,...pt]);const rem=hand.filter((_,i)=>!sel.includes(i));const safe=pool.filter(t=>t.t!=="j");const inc=safe.slice(0,sel.length);setPool(safe.slice(sel.length));setHand([...rem,...inc]);setSel([]);setNewIdx([]);setPhase("chooseHand");}}
      ...
      {phase==="chooseHand"&&(
        <>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            <h2 style={{fontFamily:F.d,fontSize:18,color:C.ink,margin:0}}>What hand are you playing?</h2>
            {getDisplayTime()&&<span aria-label={`Time elapsed: ${getDisplayTime()}`} style={{fontSize:12,color:C.mut,fontFamily:F.d,fontWeight:700}}>⏱ {getDisplayTime()}</span>}
          </div>
      ...
      {phase==="pass"&&(
        <>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
            ...
            <span style={{fontFamily:F.d,fontSize:13,fontWeight:700,color:C.mut}}>
              {mode==="daily"?`Daily #${getDayNum()}`:"Practice"}
            </span>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              {getDisplayTime()&&<span aria-label={`Time elapsed: ${getDisplayTime()}`} style={{fontSize:12,color:C.mut,fontFamily:F.d,fontWeight:700}}>⏱ {getDisplayTime()}</span>}
            </div>
          </div>
          {dd&&pi===0&&cn===1&&!hasNew&&<div role="status" style={{textAlign:"center",marginBottom:6}}>
            <span style={{fontSize:10,color:dd.c,fontWeight:600,background:dd.c+"10",padding:"3px 10px",borderRadius:8,border:`1px solid ${dd.c}20`}}>{dd.e} {dd.l}</span>
          </div>}
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
...
function Ask({icon,title,desc,hand,timer,onNo,onYes,onSort,large}){
  return(
    <div style={S.pg} className="rk-pg">
      <div style={{display:"flex",justifyContent:"flex-end",marginBottom:4}}>{timer&&<span aria-label={`Time elapsed: ${timer}`} style={{fontSize:12,color:C.mut,fontFamily:F.d,fontWeight:700}}>⏱ {timer}</span>}</div>
