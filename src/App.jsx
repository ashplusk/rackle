...
@media (prefers-reduced-motion: reduce){.rk-hero-live,.rk-soft-glow,.rk-count-live,.rk-streak-copy,.rk-float,.rk-pulse{animation:none!important}}
`;
const S={
  ...
  return(
    <div style={S.pg} className="rk-pg">
      ...
      <div style={{textAlign:"center",paddingTop:8,marginBottom:20}}>
        ...
        <h2 style={{fontFamily:F.d,fontSize:22,color:C.ink,margin:"0 0 10px",fontWeight:900,letterSpacing:-0.35,lineHeight:1.05}}>{st.title}</h2>
    ...
    <div style={{marginBottom:0}}>
      <button onClick={()=>savedClub?setOpen(o=>!o):setScreen("clubs")} style={{width:"100%",display:"flex",alignItems:"flex-start",justifyContent:"space-between",padding:"16px 14px",borderRadius:open?"0":"0 0 12px 12px",background:C.jade+"06",border:"none",borderTop:`1px solid ${C.bdr}`,cursor:"pointer",textAlign:"left"}}>
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontFamily:F.d,fontSize:16,fontWeight:900,color:C.ink,lineHeight:1.14,letterSpacing:-0.2,marginBottom:10}}>{savedClub?savedClub.name:"Club Leaderboard"}</div>
          {/* Live stat pills for club members */}
          {savedClub&&clubStats?(
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              <span style={{fontSize:10,fontWeight:700,color:C.jade,background:C.jade+"12",borderRadius:20,padding:"2px 10px"}}>{clubStats.total} club scores today</span>
    ...
    <div id="global-leaderboard" style={{marginBottom:0}}>
      <button onClick={toggle} style={{width:"100%",display:"flex",alignItems:"flex-start",justifyContent:"space-between",padding:"16px 14px",borderRadius:open?"12px 12px 0 0":"12px 12px 0 0",background:"#2460A806",border:"none",borderBottom:open?"none":`1px solid #2460A815`,cursor:"pointer",textAlign:"left"}}>
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontFamily:F.d,fontSize:16,fontWeight:900,color:C.ink,lineHeight:1.14,letterSpacing:-0.2,marginBottom:10}}>Rackle Leaderboard</div>
          {/* Live stat pills */}
          {hasData?(
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              <span style={{fontSize:10,fontWeight:700,color:"#2460A8",background:"#2460A812",borderRadius:20,padding:"2px 10px"}}>{entries.length} scores posted today</span>
...
function TodayRackleHeroCard({dn,onPlay,ds,club,clubPlayers,bestIQ,ydIQ,weekDelta,streak=0}){
  ...
  const chips=[
    {label:"players in today",value:liveCount||"Live"},
    bestIQ?.score?{label:"your best",value:bestIQ.score}:null,
    topScore?{label:club?"club leader":"top today",value:topScore}:null,
    {label:currentStreak>0?"streak alive":"starts today",value:currentStreak>0?`${currentStreak}d`:"Day 1"},
  ].filter(Boolean).slice(0,4);
  return(
    <button onClick={onPlay} aria-label={`Play Daily Rackle challenge number ${dn}`} className="rk-in rk-hero-live" style={{
      ...
      <div style={{position:"relative",padding:"20px 18px 17px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:16}}>
          ...
          <div style={{width:56,height:56,borderRadius:999,background:"rgba(255,255,255,0.94)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 8px 24px rgba(0,0,0,0.2)",flexShrink:0,transform:"translateY(-12px)",border:"1px solid rgba(255,255,255,0.55)"}}>
            <span style={{fontSize:21,color:C.jade,marginLeft:3}}>▶</span>
          </div>
        </div>
...
function ClubPulseCard({club,clubPlayers,clubEntries=[],currentScore=0,currentRank=null,setScreen}){
  ...
  const lines=[
    `${actualCount||1} club ${(actualCount||1)===1?"score is":"scores are"} live today`,
    topEntry?`${topEntry.name||"Club leader"} set the score to chase: ${topEntry.iqScore}`:(currentScore?`Your ${currentScore} is on the club board.`:"Your club board is waiting for today's scores."),
...
function TomorrowPreviewCard(){
  ...
  const headline=urgency?"Last call before the next rack.":"A new rack drops soon.";
    ...
    return(
      <div aria-label="Tomorrow's Rackle preview" style={{width:"100%",margin:"0 0 20px",border:`1px solid ${C.bdr}`,background:"linear-gradient(135deg,#FBF7EF,#F7F2EA)",borderRadius:18,padding:"17px 18px",display:"flex",alignItems:"center",gap:14,cursor:"default",textAlign:"left",boxShadow:"0 4px 16px rgba(0,0,0,0.026)",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{flex:1,minWidth:0,position:"relative"}}>
          ...
          <div style={{fontFamily:F.d,fontSize:16,fontWeight:900,color:C.ink,lineHeight:1.1,letterSpacing:-0.2,marginBottom:5}}>New rack in {resetLabel}</div>
